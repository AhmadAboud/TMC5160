...
HX711_ADC::HX711_ADC(uint16_t dout,GPIO_TypeDef* portDout, uint16_t sck,GPIO_TypeDef* portSck) //constructor
{
	doutPin = dout;
	sckPin = sck;
}
...
int HX711_ADC::startMultiple(unsigned long t)
{
	...
	if(startStatus == 0) {
		...
		if((t1 - startMultipleTimeStamp) < startMultipleWaitTime) {
			...
			static unsigned long timeout = t1 + tareTimeOut;
			doTare = 1;
			update();
			if(convRslt == 2)
			{
...
int HX711_ADC::startMultiple(unsigned long t, bool dotare)
{
	...
	if(startStatus == 0) {
		if(isFirst) {
			startMultipleTimeStamp = DWT->CYCCNT;
			if (t < 400)
			{
				startMultipleWaitTime = t + 400; //min time for HX711 to be stable
			}
...
void HX711_ADC::tare()
{
	...
	CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;
		...
		unsigned long t1 = DWT->CYCCNT;
	unsigned long timeout = t1 + tareTimeOut;
	while(rdy != 2)
	{
		rdy = update();
		if (!tareTimeoutDisable)
...
uint8_t HX711_ADC::update()
{
	Pinout* mPinout;
	mPinout = new Pinout();
	double dout = mPinout->doutPin->Read(); //check if conversion is ready
	...
	else
	{
		...
		if (DWT->CYCCNT - lastDoutLowTime > SIGNAL_TIMEOUT)
		{
			signalTimeoutFlag = 1;
		}
		convRslt = 0;
...
{
	if (dataWaiting) { lastDoutLowTime = DWT->CYCCNT; return 1; }
	Pinout* mPinout;
	mPinout = new Pinout();
	double dout = mPinout->doutPin->Read();
	if (!dout)
	{
		dataWaiting = true;
		lastDoutLowTime = DWT->CYCCNT;
		signalTimeoutFlag = 0;
		return 1;
	}
	else
...
void HX711_ADC::conversion24bit()  //read 24 bit data, store in dataset and start the next conversion
{
	...
	{ 	//read 24 bit data + set gain and start next conversion
		...
		mPinout = new Pinout();
		mPinout->sckPin->Set();
		if(SCK_DELAY) HAL_Delay(1); // could be required for faster mcu's, set value in config.h
		mPinout->sckPin->Set();
		if (i < (24))
		{
			dout = mPinout->doutPin->Read();
			data = (data << 1) | dout;
		} else {
			if(SCK_DELAY) HAL_Delay(1); // could be required for faster mcu's, set value in config.h
		}
...
bool HX711_ADC::refreshDataSet()
{
	...
	while ( s > 0 ) {
		...
		Pinout* mPinout;
		mPinout = new Pinout();
		mPinout->doutPin->Read();

		if (mPinout->doutPin->Read() == 0) { // HX711 dout pin is pulled low when a new conversion is ready
