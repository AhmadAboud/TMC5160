...
void hx7111_init(hx711_tt *hx7111, GPIO_TypeDef *clk_gpio, uint16_t clk_pin, GPIO_TypeDef *dat_gpio, uint16_t dat_pin){
  ...
  hx7111->clk_gpio = clk_gpio;
  hx7111->clk_pin = clk_pin;
  hx7111->dat_gpio = dat_gpio;
  hx7111->dat_pin = dat_pin;
...
//#############################################################################################
void set_scale(hx711_tt *hx711, float Ascale, float Bscale){
...
//#############################################################################################
void set_gain(hx711_tt *hx711, uint8_t Again, uint8_t Bgain){
...
//#############################################################################################
void set_offset(hx711_tt *hx711, long offset, uint8_t channel){
	if(channel == CHANNEL_A) hx711->Aoffset = offset;
	else hx711->Boffset = offset;
}

//############################################################################################
uint8_t shiftIn(hx711_tt *hx711, uint8_t bitOrder) {
    uint8_t value = 0;
    uint8_t i;
...
//############################################################################################
void wait_ready(hx711_tt *hx711) {
...
//############################################################################################
long read(hx711_tt *hx711, uint8_t channel){
	wait_ready(hx711);
...
//############################################################################################
long read_average(hx711_tt *hx711, int8_t times, uint8_t channel) {
	long sum = 0;
...
//############################################################################################
double get_value(hx711_tt *hx711, int8_t times, uint8_t channel) {
	long offset = 0;
...
//############################################################################################
void tare_all(hx711_tt *hx711, uint8_t times) {
	tare(hx711, times, CHANNEL_A);
	tare(hx711, times, CHANNEL_B);
}

//############################################################################################
float get_weight(hx711_tt *hx711, int8_t times, uint8_t channel) {
  // Read load cell
	read(hx711, channel);
	float scale = 0;
